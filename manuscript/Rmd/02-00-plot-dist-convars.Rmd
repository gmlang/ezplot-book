...
```{r, hist_budget, fig.cap="Histogram of budget", dpi=300, fig.width=6, fig.height=6}
...
```{r, density_budget, fig.cap="Density plot of budget", dpi=300, fig.width=6, fig.height=6}
plt("budget", type = 'density') 
```

Instead of looking at its density curve, we can plot its Cumulative Distribution Function (CDF).

```{r, cdf_budget, fig.cap="CDF plot of budget", dpi=300, fig.width=6, fig.height=6}
...
```{r, box_budget, fig.cap="Boxplot of budget", dpi=300, fig.width=6, fig.height=6}
...
```{r, qq_budget, fig.cap="Normal probability plot of budget", dpi=300, fig.width=6, fig.height=6}
plt = mk_qqplot(films) 
plt("budget") 
```
...
```{r, hist_log_budget, fig.cap="Histogram of log(budget)", dpi=300, fig.width=6, fig.height=6}
films$log_budget = log(films$budget)
plt2 = mk_histdens(films) 
plt2("log_budget", bins = 100) # draw histogram by default
```

```{r, density_log_budget, fig.cap="Density of log(budget)", dpi=300, fig.width=6, fig.height=6}
plt2("log_budget", type = 'density', adjust = 0.6) 
```

```{r, cdf_log_budget, fig.cap="CDF of log(budget)", dpi=300, fig.width=6, fig.height=6}
plt2 = mk_cdfplot(films)
plt2("log_budget", add_vline_median = TRUE)
```

```{r, boxplot_log_budget, fig.cap="Boxplot of log(budget)", dpi=300, fig.width=6, fig.height=6}
plt2 = mk_boxplot(films)
plt2(yvar = "log_budget")
```

```{r, qq_log_budget, fig.cap="Normal probability plot of log(budget)", dpi=300, fig.width=6, fig.height=6}
