...
```{r, check_exp_time_bt_births, fig.cap="Does time between births follow the exponential distribution?"}
...
```{r, check_norm_sepallen, fig.cap="Does sepal length follow the normal distribution?"}
...
```{r, check_lognorm_pop, fig.cap="Does population (< 100 million) follow the lognormal distribution?"}
...
```{r, check_pareto_pop, fig.cap="Does population (100+ million) follow the pareto distribution?"}
# focus on cities/towns with 100+ million people
df = pops %>% filter(!is.na(pop), pop >= 10^5) # pop is in thousands
f = check_pareto(df)
